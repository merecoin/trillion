...
#if defined(NDEBUG)
# error "Trillion cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x80abb3d39b31df3bb78ecd6274f44e7084af778ae7b5fdd4ba576656c82d63c5");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Trillion: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Trillion Signed Message:\n";
...
{
    // Trillion: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 4000000 * COIN;
	
	if(nHeight == 2)  
    {
        nSubsidy = 4000000000000 * COIN;
    }
    else if(nHeight == 4)  
    {
        nSubsidy = 2280000000000 * COIN;
    }

    // Subsidy is cut in half every 840000 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 840000); // Trillion: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Trillion: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Trillion: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Trillion: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Trillion: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xc6cde074a14995aa8599ff688e2033970f289c24d8d189ea06f5d2a17a6a4c63");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "No man is an island";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("040b88619acfbadbeca1f225a397450b63f5bb8f7e4e9b3b75ca116de46252f8be05f4a1aad917fdb3b634db901a1f54028bbd43f517bf0cb1dc939c1b25b0d0c9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1525691279;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1363829;

        if (fTestNet)
        {
            block.nTime    = 1525691304;
            block.nNonce   = 3802164;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xff13ab3f51dcec5977b4ff54b2ec21c9c6699e0ecbb4bdb846236821689f1e40"));
...
unsigned char pchMessageStart[4] = { 0xc1, 0x54, 0x82, 0x80 }; // Trillion: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("TrillionMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("TrillionMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static TrillionMiner(CWallet *pwallet)
{
    printf("TrillionMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("trillion-miner");
    ...
    try { loop {
        ...
        printf("Running TrillionMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("TrillionMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&TrillionMiner, pwallet));
}
